...
type OpenUrl =
    ...
    static member Open(url: string) =
        if isNull OpenUrl.__signature_Open_589601652 then
            OpenUrl.__signature_Open_589601652 <-
                JSFunctionBinding.BindJSFunction(
                    "globalThis.window.open",
                    null,
                    [| JSMarshalerType.Discard; JSMarshalerType.String |]
                        .AsSpan()
                )

        let __arguments_buffer =
            System.Span<JSMarshalerArgument>(NativePtr.stackalloc<JSMarshalerArgument> 3 |> NativePtr.toVoidPtr, 3)
