...
module ThumbPage =
    ...
    let getRect () =
        let topLeft = Point(Canvas.GetLeft(partTopLeftRef.Value), Canvas.GetTop(partTopLeftRef.Value))
        let topRight = Point(Canvas.GetLeft(partTopRightRef.Value), Canvas.GetTop(partTopRightRef.Value))
        let bottomLeft = Point(Canvas.GetLeft(partBottomLeftRef.Value), Canvas.GetTop(partBottomLeftRef.Value))
        let bottomRight = Point(Canvas.GetLeft(partBottomRightRef.Value), Canvas.GetTop(partBottomRightRef.Value))
        let left = Math.Min(Math.Min(topLeft.X, topRight.X), Math.Min(bottomLeft.X, bottomRight.X))
        let top = Math.Min(Math.Min(topLeft.Y, topRight.Y), Math.Min(bottomLeft.Y, bottomRight.Y))
        let right = Math.Max(Math.Max(topLeft.X, topRight.X), Math.Max(bottomLeft.X, bottomRight.X))
        let bottom = Math.Max(Math.Max(topLeft.Y, topRight.Y), Math.Max(bottomLeft.Y, bottomRight.Y))
    ...
    let updateDrag (rect: Rect) =
        if partDragRef.TryValue.IsSome
        then
    ...
    let update msg model =
        ...
        | OnLoaded args ->
            if controlRef.TryValue.IsSome then
                ...
                { LeftOffset = Canvas.GetLeft(controlRef.Value); TopOffset = Canvas.GetTop(controlRef.Value) }
            else
                model
