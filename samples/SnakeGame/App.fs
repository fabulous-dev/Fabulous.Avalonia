...
// Credits to https://github.com/wiredsister/Snakefs
module SnakeGame =
    ...
    module Graphics =
        ...
        let isFreeSpace square =
            square = display || square = errorDisplay
        ...
        let makeGrid (dimensionsSquared) : Grid =
            let row _ =
                Array.init dimensionsSquared (fun _ -> display)

            Array.init dimensionsSquared row

        let clearGrid (grid: Grid) : unit =
            Array.iteri (fun i row -> Array.iteri (fun j _ -> grid.[i].[j] <- display) row) grid
        ...
        let getFreeSpaces (grid: Grid) : list<Position> =
            ...
            for i in 0 .. (grid.Length - 1) do
                for j in 0 .. (grid.Length - 1) do
                    if isFreeSpace grid.[i].[j] then
                        results.Value <- results.Value @ [ i, j ]
                    else
                        ()
        ...
        let getFood (grid: Grid) : Food =
            Console.Beep()
            let freeSpaces = getFreeSpaces grid |> Array.ofList
            let food = randomFood()
        ...
        let slither (snake: Snake<Position>) (grid: Grid) : unit =
            try
                ...
                do slithering snake
            with _ ->
                failwith "ERROR: Could not slither snake!"

        let endGame (grid: Grid) : unit =
            Console.Clear()
            Array.iteri (fun i row -> Array.iteri (fun j _ -> grid.[i].[j] <- errorDisplay) row) grid
    ...
    module GamePlay =
        ...
        let moveUp (snake: Snake<Position>) (grid: Grid) (eaten: int) (food: Food) : Game =
            ...
            | Head(p, rest: Snake<Position>) ->
                ...
                try
                    match shiftUp with
                    | (row, column) when Graphics.isOccupied grid.[row].[column] -> GameOver(grid, eaten)
...
module App =
    ...
    let update msg model =
        match msg with
        | Evolve -> { model with Count = model.Count + 1 }, Cmd.none
    ...
    let view model =
        ...
        //             | keystroke when keystroke.Key.Equals(ConsoleKey.UpArrow) -> return GamePlay.moveUp snake grid eaten food
        //             | keystroke when keystroke.Key.Equals(ConsoleKey.DownArrow) -> return GamePlay.moveDown snake grid eaten food
        //             | keystroke when keystroke.Key.Equals(ConsoleKey.RightArrow) -> return GamePlay.moveRight snake grid eaten food
        //             | keystroke when keystroke.Key.Equals(ConsoleKey.LeftArrow) -> return GamePlay.moveLeft snake grid eaten food
        //             | _ ->
        //                 match prev with
        //                 | Up _ -> return GamePlay.moveUp snake grid eaten food
        //                 | Down _ -> return GamePlay.moveDown snake grid eaten food
