...
module AnimationBuilders =
    ...
    type Fabulous.Avalonia.View with
        ...
        static member Animation<'msg>(duration: TimeSpan, iterationCount: IterationCount) =
            CollectionBuilder<'msg, IFabAnimation, IFabKeyFrame>(
                Animation.WidgetKey,
                Animation.Children,
                Animation.Duration.WithValue(duration),
                Animation.IterationCount.WithValue(iterationCount)
            )

[<Extension>]
type AnimationModifiers =
    [<Extension>]
    static member inline playbackDirection(this: WidgetBuilder<'msg, #IFabKeyFrame>, value: PlaybackDirection) =
        this.AddScalar(Animation.PlaybackDirection.WithValue(value))

    [<Extension>]
    static member inline fillMode(this: WidgetBuilder<'msg, #IFabKeyFrame>, value: FillMode) =
        this.AddScalar(Animation.FillMode.WithValue(value))

    [<Extension>]
    static member inline easing(this: WidgetBuilder<'msg, #IFabKeyFrame>, value: Easing) =
        this.AddScalar(Animation.Easing.WithValue(value))

    [<Extension>]
    static member inline delay(this: WidgetBuilder<'msg, #IFabKeyFrame>, value: TimeSpan) =
        this.AddScalar(Animation.Delay.WithValue(value))

    [<Extension>]
    static member inline delayBetweenIterations(this: WidgetBuilder<'msg, #IFabKeyFrame>, value: TimeSpan) =
        this.AddScalar(Animation.DelayBetweenIterations.WithValue(value))

    [<Extension>]
    static member inline speedRatio(this: WidgetBuilder<'msg, #IFabKeyFrame>, value: float) =
