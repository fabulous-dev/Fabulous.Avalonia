...
module ScrollViewer =
    ...
    let ScrollChangedEvent = 
        Attributes.defineEvent
            "ScrollViewer_ScrollChangedEvent"
            (fun target -> (target :?> ScrollViewer).ScrollChanged)
...
type ScrollViewerModifiers =
    ...
    [<Extension>]
    static member inline isScrollChainingEnabled(this: WidgetBuilder<'msg, #IFabScrollViewer>, value: bool) =
    ...
    static member inline onScrollChanged(this: WidgetBuilder<'msg, #IFabScrollViewer>, onScrollChanged: ScrollChangedEventArgs -> 'msg) =
        this.AddScalar(ScrollViewer.ScrollChangedEvent.WithValue(fun args -> onScrollChanged args |> box))
