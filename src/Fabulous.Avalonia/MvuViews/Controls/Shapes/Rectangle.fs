...
module MvuRectangleBuilders =
    type Fabulous.Avalonia.Mvu.View with
        ...
        static member Rectangle(radiusX: float, radiusY: float) =
            WidgetBuilder<'msg, IFabMvuRectangle>(Rectangle.WidgetKey, Rectangle.RadiusX.WithValue(radiusX), Rectangle.RadiusY.WithValue(radiusY))

        /// <summary>Creates a Rectangle widget.</summary>
        static member Rectangle() =
            WidgetBuilder<'msg, IFabMvuRectangle>(Rectangle.WidgetKey, AttributesBundle(StackList.empty(), ValueNone, ValueNone))
